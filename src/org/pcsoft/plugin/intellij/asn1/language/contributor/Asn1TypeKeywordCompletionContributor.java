 ...
 */
public class Asn1TypeKeywordCompletionContributor extends CompletionContributor {
    public Asn1TypeKeywordCompletionContributor() {
        extend(
                CompletionType.BASIC,
                PlatformPatterns.or(
                        PlatformPatterns.psiElement().afterLeaf(PlatformPatterns.psiElement(Asn1CustomElementFactory.BRACES_CURLY_CLOSE).withParent(Asn1ObjectClassDefinition.class)).withLanguage(Asn1Language.INSTANCE)
                ),
                new CompletionProvider<CompletionParameters>() {
                    @Override
                    protected void addCompletions(@NotNull CompletionParameters completionParameters, ProcessingContext processingContext, @NotNull CompletionResultSet completionResultSet) {
                        completionResultSet.addAllElements(Arrays.asList(
                                LookupElementBuilder.create("WITH SYNTAX")
